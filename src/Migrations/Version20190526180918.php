 ...
 */
final class Version20190526180918 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TABLE pre_order (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER NOT NULL, status VARCHAR(255) NOT NULL, date DATETIME NOT NULL)');
        $this->addSql('CREATE INDEX IDX_EF82FC73A76ED395 ON pre_order (user_id)');
        $this->addSql('CREATE TABLE pre_order_product (pre_order_id INTEGER NOT NULL, product_id INTEGER NOT NULL, PRIMARY KEY(pre_order_id, product_id))');
        $this->addSql('CREATE INDEX IDX_DFFFC6D38B495F6B ON pre_order_product (pre_order_id)');
        $this->addSql('CREATE INDEX IDX_DFFFC6D34584665A ON pre_order_product (product_id)');
        ...
        $this->addSql('CREATE TABLE basket (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id_id INTEGER NOT NULL, status VARCHAR(255) NOT NULL COLLATE BINARY, CONSTRAINT FK_2246507B9D86650F FOREIGN KEY (user_id_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('DROP TABLE pre_order_product');
        $this->addSql('DROP INDEX IDX_2246507B9D86650F');
        $this->addSql('CREATE TEMPORARY TABLE __temp__basket AS SELECT id, user_id_id, status FROM basket');
        $this->addSql('DROP TABLE basket');
